...
function runClientsOneMachine() {
    ...
    do
        ...
        then
            "${EPaxosFolder}"/bin/clientnew -maddr ${MasterIp} -q ${reqsNb} -w ${writes} -e=true -r ${rounds} -p 30 -c ${conflicts} > ${LogFolder}/S${NumOfServerInstances}-C${NumOfClientInstances}-q${reqsNb}-w${writes}-r${rounds}-c${conflicts}--client${idx}.out 2>&1 &
        fi
    done
}
