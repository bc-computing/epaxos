...
BASE_REMOTE_BIN_DIRECTORY_NFS="${SRC_DIRECTORY}" # Client/server binaries are stored in <user>/epaxos. Note: this location is created with mkdir without the -p flag, so you have to store the binary in a folder inside <user>/.
